...
class SelectFromLeafExecutor(object):
    ...
    def execute(self, es_url, arguments=None):
        ...
        if self.sql_select.from_table.startswith('_cluster_health'):
            response = json.loads(urllib2.urlopen('%s/_cluster/health' % es_url).read())
            response = {'hits': {'hits': [{'_source': response}]}}
        elif self.sql_select.from_table.startswith('_cluster_state'):
            _, _, metric = self.sql_select.from_table.partition('.')
            if metric == 'nodes':
